    ...
    , m_sink(nullptr)
{
    ...
    gtk_widget_add_events(m_video_area, GDK_BUTTON_PRESS_MASK | GDK_STRUCTURE_MASK);
    
    g_signal_connect(m_video_area, "button-press-event", G_CALLBACK(on_button_press), this);
    g_signal_connect(m_video_area, "size-allocate", G_CALLBACK(on_size_allocate), this);
    ...
    g_object_set(m_sink, "force-aspect-ratio", FALSE, "handle-events", FALSE, nullptr);
}
...
GtkWidget* video_preview::get_window() const
{
    return m_window;
}

GstElement* video_preview::get_sink() const
{
    return m_sink;
}
...
void video_preview::on_size_allocate(GtkWidget* widget, GtkAllocation* allocation, gpointer data)
{
    video_preview* self = static_cast<video_preview*>(data);
...
void video_preview::handle_size_allocate(GtkAllocation* allocation)
{
    if (m_sink) {
        GstVideoOverlay* overlay = GST_VIDEO_OVERLAY(m_sink);
        gst_video_overlay_set_render_rectangle(overlay, allocation->x, allocation->y, allocation->width, allocation->height);
    }
}

gboolean video_preview::on_button_press(GtkWidget* widget, GdkEventButton* event, gpointer data)
{
    video_preview* self = static_cast<video_preview*>(data);
...
gboolean video_preview::handle_button_press(GdkEventButton* event)
{
    if (event->type == GDK_BUTTON_PRESS && event->button == 3) {  // Right click
        ...
        GtkWidget* restore_item = gtk_menu_item_new_with_label("Restore Aspect Ratio");
        g_signal_connect(restore_item, "activate", G_CALLBACK(on_restore_aspect_ratio), this);
        gtk_menu_shell_append(GTK_MENU_SHELL(menu), restore_item);
        
        GtkWidget* free_item = gtk_menu_item_new_with_label("Free Aspect Ratio");
        g_signal_connect(free_item, "activate", G_CALLBACK(on_free_aspect_ratio), this);
...
void video_preview::on_restore_aspect_ratio(GtkMenuItem* menu_item, gpointer data)
{
    video_preview* self = static_cast<video_preview*>(data);
...
void video_preview::on_free_aspect_ratio(GtkMenuItem* menu_item, gpointer data)
{
    video_preview* self = static_cast<video_preview*>(data);
